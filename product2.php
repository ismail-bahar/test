...
class ModelJournal3Product extends Model {
	...
	public function getProduct($product_id) {
		...
		foreach ($query->rows as $row) {
			...
			'price'            =>  $row['price']
				= $this->conversationPriceCurrency($query->row['price'], $query->row['price_currency_code']),
		...
		//$query->row['price'] = $this->conversationPriceCurrency($query->row['price'], $query->row['price_currency_code']);
